%Branch_and_Bound_function for positive
%Created by E T Wang
%2019.4.15
%debug OK��!

function [A_M,x_opt,zLB] = B_and_B_func_positive(c,A,b,Aeq,beq)
    [~,n]=size(A);
    %%all 0<=x<=1,here don't need it
%     x_lb=zeros(n,1);
%     x_ub=ones(n,1);
    %%test,-c:max-->min
    ...
    while active_node<=n_count
        ...
        [x_j,z_j,exitFlag]=linprog(c,A,b,Aeq,beq);
        z_j=-z_j;%%min->max
    ...
    elseif z_j>zLB && max(abs(x_j-round(x_j)))>=1.0e-7
        ...
        %branching -->
        [x_j_m,x_j_n]=size(x_j);
        temp=0;
        for j=1:x_j_m
            if x_j(j,:)~=floor(x_j(j,:))
                temp=floor(x_j(j,:));
                break;
            end
        end
        branch_L(1,2+x_cut)=temp;
        branch_R(1,2+x_cut)=temp+1;
